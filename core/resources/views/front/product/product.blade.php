            ...
            <div class="col-lg-3 col-md-5 col-sm-7 order-2 order-lg-1">
                <div class="shop-sidebar">
                    ...
                    @if(isset($productType) && !empty($productType))
                        ...
                        <div class="shop-dropdown mt-30 text-right" style="margin-top:20px">
                            <select name="ptype" id="ptype_sort">
                                <option value="">Product type</option>
                                @foreach($productType as $key=>$v)
                                    <option value="{{$key}}" {{ request()->input('ptype') == $key ? 'selected' : '' }}>{{$v}}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    @if(request()->get('category_id') && !empty(request()->get('category_id')))
                        ...
                        <?php
                            ...
                            $subCategoryData = \App\PsubCategory::where('status',1)
                                            ->has('productSubCategoryData')
                                            ->pluck('name','id')
                                            ->toArray();
                        ...
                        @if(isset($subCategoryData) && !empty($subCategoryData))
                            <div class="shop-box shop-tag mt-30">
                                ...
                                <div class="tag-item">
                                    <ul>
                                        <li class="{{ request()->input('scategory') == '' ? 'active-search' : '' }}"><a data-href="" class="scategory-id cursor-pointer">{{__('All')}}</a></li>
                                        @foreach ($subCategoryData as $key=>$tag)
                                            <li class="{{ request()->input('scategory') == $key ? 'active-search' : '' }}"><a data-href="{{$key}}" class="scategory-id cursor-pointer">{{convertUtf8($tag)}}</a></li>
                                        @endforeach
                                    </ul>
                                </div>
                            </div>
                    ...
                    <div class="shop-item">
                        <div class="shop-thumb">
                            ...
                            <ul>
                                ...
                                @else
                                    <li><a href="tel:{{$product->phone_number}}" data-toggle="tooltip" data-placement="top" title="Call for price"><i class="fas fa-phone"></i></a></li>
                                @endif
...
<form id="searchForm" class="d-none"  action="{{ route('front.product') }}" method="get">
    ...
    <input type="hidden" name="scategory" id="scategory" value="{{ !empty(request()->input('scategory')) ? request()->input('scategory') : '' }}">
    <input type="hidden" name="ptype" id="ptype_search" value="{{ !empty(request()->input('ptype')) ? request()->input('ptype') : '' }}">
...
$(document).on('change','#ptype_sort',function(){
    ptypeSort = $(this).val();
    $('#ptype_search').val(ptypeSort);
    $('#search-button').click();
})
...
$(document).on('click','.scategory-id',function(){
    scategory = '';
    if($(this).attr('data-href') != 0){
        scategory = $(this).attr('data-href');
    }
