                    ...
                    <tbody>
                      @foreach ($aulinks as $key => $aulink)
                        <tr>
                          ...
                          <td>
                            {{ isset($aulink->footerGroupName) ? $aulink->footerGroupName->name : '-'}}
                          </td>
                          <td>{{$aulink->url}}</td>
                          <td>
                            <a class="btn btn-secondary btn-sm editbtn" href="#editModal" data-toggle="modal" data-ulink_id="{{$aulink->id}}" data-name="{{$aulink->name}}" data-url="{{$aulink->url}}" data-groupname="{{$aulink->footer_group_link_name_id}}">
        ...
        <div class="modal-body">
          <form id="ajaxForm" class="modal-form create" action="{{route('admin.ulink.store')}}" method="POST">
            ...
            <div class="form-group">
              ...
              <select class="form-control" name="footer_group_link_name_id">
                <option value=""> Select group by name</option>
                @foreach($groupName as $key=>$v)
                  <option value="{{$v->id}}">{{$v->name}}</option>
                @endforeach
              </select>
              <p id="errfooter_group_link_name_id" class="mb-0 text-danger em"></p>
            </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="ajaxEditForm" action="{{route('admin.ulink.update')}}" method="POST">
            ...
            <div class="form-group">
              <label for="">Group By</label>
              <select class="form-control" name="footer_group_link_name_id" id="ingroupname">
                <option value=""> Select group by name</option>
                @foreach($groupName as $key=>$v)
                  <option value="{{$v->id}}">{{$v->name}}</option>
                @endforeach
