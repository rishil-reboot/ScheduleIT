...
<div class="page-header">
   <h4 class="page-title">Edit Customer Faqs</h4>
   ...
   <div class="col-md-12">
      <div class="card">
         <div class="card-header">
            <div class="card-title d-inline-block">Edit Customer Faqs</div>
            @if ($faq->language_id > 0)
            <a class="btn btn-info btn-sm float-right d-inline-block" href="{{ route('admin.customer-faq.index') . '?language=' . request()->input('language') }}">
            ...
            @else
            <a class="btn btn-info btn-sm float-right d-inline-block" href="{{ route('admin.customer-faq.index') }}">
            <span class="btn-label">
            <i class="fas fa-backward" style="font-size: 12px;"></i>
            </span>
            Back
         ...
         <div class="card-body pt-5 pb-5">
            <div class="row">
               <div class="col-lg-6 offset-lg-3">
                  <form class="mb-3 dm-uploader drag-and-drop-zone" enctype="multipart/form-data" action="{{route('admin.customer-faq.uploadUpdate', $faq->id)}}" method="POST">
                     @csrf
                     <div class="form-row px-2">
                        ...
                        <div class="col-md-12 d-md-block d-sm-none mb-3">
                           <img src="{{asset('assets/front/img/faq/customer/'.$faq->image)}}" alt="..." class="img-thumbnail">
                        </div>
                        <div class="col-sm-12">
                  ...
                  <form id="ajaxForm" class="" action="{{route('admin.customer-faq.update')}}" method="post">
                     @csrf

                     <div class="form-group">
                        <label for="">Category **</label>
                        <select class="form-control" name="faq_category_id" id="bcategory">
                          <option value="" selected disabled>Select a category</option>
                          @foreach ($bcats as $key => $bcat)
                            <option value="{{$bcat->id}}" {{$bcat->id == $faq->faqCategory->id ? 'selected' : ''}}>{{$bcat->name}}</option>
                          @endforeach
                        </select>
