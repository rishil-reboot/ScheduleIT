...
@section('content')
  <div class="page-header">
    <h4 class="page-title">Product Sub Categories</h4>
    ...
    <div class="col-md-12">
      ...
      <div class="card">
        <div class="card-header">
            <div class="row">
                <div class="col-lg-4">
                    <div class="card-title d-inline-block">Sub Categories</div>
                </div>
                <div class="col-lg-3">
                    @if (!empty($langs))
                ...
                <div class="col-lg-4 offset-lg-1 mt-2 mt-lg-0">
                    <a href="#" class="btn btn-primary float-right btn-sm" data-toggle="modal" data-target="#createModal"><i class="fas fa-plus"></i> Add Sub Category</a>
                    <button class="btn btn-danger float-right btn-sm mr-2 d-none bulk-delete" data-href="{{route('admin.sub-category.bulk.delete')}}"><i class="flaticon-interface-5"></i> Delete</button>
                </div>
            </div>
        </div>
        <div class="card-body">
                    ...
                    <tbody>
                      @foreach ($pcategories as $key => $category)
                        <tr>
                          ...
                          <td>{{convertUtf8($category->category->name)}}</td>
                          ...
                          <td>
                            <a class="btn btn-secondary btn-sm editbtn" href="{{route('admin.sub-category.edit', $category->id) . '?language=' . request()->input('language')}}">
                              <span class="btn-label">
                                <i class="fas fa-edit"></i>
                              </span>
                              Edit
                            </a>
                            <form class="deleteform d-inline-block" action="{{route('admin.sub-category.delete')}}" method="post">
                              @csrf
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="ajaxForm" class="modal-form" action="{{route('admin.sub-category.store')}}" method="POST">
            @csrf
    ...
    $("select[name='language_id']").on('change', function() {
        ...
        $.get(url, function(data) {
            ...
            if (data == 1) {
             ...
             let langid = $(this).val();
             let urlCat = "{{url('/')}}/admin/sub-category/" + langid + "/getcats";
             // console.log(url);
