...
class InboxwebmailEmailParser
{
     ...
     * @throws \Exception
     */
    public function __construct($host, $login, $password, $uid, $d_from_server,$sync_type)
    {
        ...
        if ($emails != false) {
            ...
            foreach ($emails as $email_number) {
                try {
                    ...
                    $email_body = $this->inboxWebmail_parse_clean_html($email_body);
                    ...
                    if ($this->from != '' && $this->subject != '' && $email_body != '') {
                        ...
                        if (strtotime($checkDate) < strtotime($this->created)) {
                            ...
                            $data->e_cc = $this->cc;
                            $data->e_bcc = $this->bcc;
                            $data->e_subject = strip_tags($this->subject);
    ...
    private function inboxWebmail_parse_clean_html($data){
        ...
        do
        {
            ...
            $data = str_ireplace(array('script','alert','javascript','prompt'), array('','','','',''), $data);
        }
        while ($old_data !== $data);
        return $data;
    }
