...
class QuoteController extends Controller
{
    ...
    /**
     * This funciton is used get quote menu text and etc related data to show in page
    ...
    public function navMenu(Request $request)
    {
        ...
        $data['pages'] = \App\Page::where('language_id', $data['lang_id'])->orderBy('id', 'DESC')->get();
        $data['abs'] = $lang->basic_setting;
        return view('admin.quote.nav-menu', $data);

    }

    /**
     * This function is used to update nav menu of quote
     * @author Chirag Ghevariya
     */
    public function updateNavMenu(Request $request,$langid) {

        $bss = BasicSetting::where('language_id', $langid)->firstOrFail();
        $bss->quote_menu_text = $request->quote_menu_text;
        $bss->quote_menu_page_id = $request->quote_menu_page_id;
        $bss->save();
        
        $request->session()->flash('success', 'Nav menu for quote updated successfully!');
        return "success";
    }
