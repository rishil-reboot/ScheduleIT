...
class TicketController extends Controller
{
    public function all(Request $request) {
        ...
        $tickets = Ticket::with(['user'])->orderby('last_message','DESC')
        ->has('user')
    ...
    public function pending(Request $request) {
        $search = $request->search;
        if(Auth::guard('admin')->user()->id == 1){
            $tickets = Ticket::with(['user'])->where('status','pending')
            ->has('user')
            ->when($search, function ($query, $search) {
                return $query->where('ticket_number', $search);
            })
    ...
    public function open(Request $request) {
        $search = $request->search;
        if(Auth::guard('admin')->user()->id == 1){
            $tickets = Ticket::with(['user'])->where('status','open')
            ->has('user')
            ->when($search, function ($query, $search) {
                return $query->where('ticket_number', $search);
            })
    ...
    public function closed(Request $request) {
        $search = $request->search;
       if(Auth::guard('admin')->user()->id == 1){
        $tickets = Ticket::with(['user'])->where('status','close')
        ->has('user')
        ->when($search, function ($query, $search) {
            return $query->where('ticket_number', $search);
        })
