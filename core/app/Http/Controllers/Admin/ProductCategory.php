...
class ProductCategory extends Controller
{
    ...
    public function delete(Request $request)
    {
        ...
        if ($category->subcategory()->count() > 0 || $category->products()->count() > 0) {
            Session::flash('warning', 'First, delete all the sub category and product under the selected categories!');
            return back();
        }
    ...
    public function bulkDelete(Request $request)
    {
        ...
        foreach ($ids as $id) {
            ...
            if ($pcategory->subcategory()->count() > 0 || $category->products()->count() > 0) {
                Session::flash('warning', 'First, delete all the sub category and product under the selected  categories!');
                return "success";
            }
        }
