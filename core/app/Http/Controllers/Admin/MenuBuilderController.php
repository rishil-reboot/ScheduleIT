...
class MenuBuilderController extends Controller
{
    ...
    public function topIndex(Request $request) {
        ...
        // get previous menus
        $menu = TopMenu::where('language_id', $lang->id)->first();
        $data['prevMenu'] = '';
        if (!empty($menu)) {
            $data['prevMenu'] = $menu->menus;
        }

        return view('admin.menu_builder.top_index', $data);
    }

    public function topUpdate(Request $request) {

        TopMenu::where('language_id', $request->language_id)->delete();
        ...
        Session::flash('success', 'Top Menu updated successfully!');
        return "success";
    }
}
