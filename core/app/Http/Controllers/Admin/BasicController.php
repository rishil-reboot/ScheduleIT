...
class BasicController extends Controller
{
    ...
    public function updatefeatrue(Request $request)
    {
        ...
        foreach ($bexs as $key => $bex) {
            ...
            $bex->is_terms_and_conditions = $request->is_terms_and_conditions;
            $bex->save();
        }
    ...
    public function updatesupport(Request $request)
    {
        ...
        foreach ($bs as $key => $v) {
            ...
            $v->support_email = $request->support_email;
            $v->support_phone = $request->support_phone;
            $v->save();
        }
    ...
    public function updateheading(Request $request, $langid)
    {
        ...
        $request->validate([
            ...
            'portfolio_subtitle' => 'nullable|max:200',
            ...
            'pricing_subtitle' => 'nullable|max:200',
            'blog_title' => 'nullable|max:30',
            'blog_subtitle' => 'nullable|max:40',
            'education_article_title' => 'nullable|max:30',
            'education_article_subtitle' => 'nullable|max:40',
            'education_article_details_title' => 'nullable|max:40',
            ...
            'feature_section_title'=>'required',
            'feature_section_subtitle'=>'required',
            'statistics_section_title'=>'required',
            'statistics_section_subtitle'=>'required',
            'steps_section_title'=>'required',
            'steps_section_subtitle'=>'required'
        ]);
        ...
        $bs->education_article_title = $request->education_article_title;
        $bs->education_article_subtitle = $request->education_article_subtitle;
        $bs->education_article_details_title = $request->education_article_details_title;
        ...
        $bs->statistics_section_title = $request->statistics_section_title;
        $bs->statistics_section_subtitle = $request->statistics_section_subtitle;
        $bs->steps_section_title = $request->steps_section_title;
        $bs->steps_section_subtitle = $request->steps_section_subtitle;
        $bs->save();
    ...
    public function updatescript(Request $request)
    {
        ...
        foreach ($bss as $bs) {
            ...
            $bs->matomo_script = $request->matomo_script;
            $bs->is_matomo = $request->is_matomo;
            ...
            $bs->google_recaptcha_site_key = CommonFunctionForMailString($request->google_recaptcha_site_key, $bs->google_recaptcha_site_key, 100);
            $bs->google_recaptcha_secret_key = CommonFunctionForMailString($request->google_recaptcha_secret_key, $bs->google_recaptcha_secret_key, 100);
    ...
    public function updatesections(Request $request)
    {
        ...
        foreach ($bss as $key => $bs) {
            ...
            $bs->client_section = $request->client_section;
            $bs->save();
        }
    ...
    /**
     * This function is used to update breadcum setting
     * @author Chirag Ghevariya
     */
    public function updateBreadcrumbSetting(Request $request, $langid)
    {
        $input = $request->all();

        $bs = BasicSetting::where('language_id', $langid)->firstOrFail();
        $bs->breadcum_type = $request->breadcum_type;
        $breadcumvideo = $request->file('breadcum_video');

        if (!empty($breadcumvideo)) {
            $allowedExts = array('mp4', 'mov');
            $rules = [
                'breadcum_video' => [
                    function ($attribute, $value, $fail) use ($breadcumvideo, $allowedExts) {
                        if (!empty($breadcumvideo)) {
                            $ext = $breadcumvideo->getClientOriginalExtension();
                            if (!in_array($ext, $allowedExts)) {
            ...
            @unlink('assets/front/img/breadcrumb/' . $bs->breadcum_video);

            $filename = time() . '.' . $breadcumvideo->getClientOriginalExtension();
            $request->file('breadcum_video')->move('assets/front/img/breadcrumb/', $filename);
            $breadcumvideo = $filename;

            $bs->breadcum_video = $breadcumvideo;
        }

        $bs->save();

        if (isset($input['padding']) && !empty($input['padding'])) {

            foreach ($input['padding'] as $key => $v) {

                $obj = \App\MyThemeVersion::where('id', $key)->first();
        ...
        if (isset($input['dashboard_padding']) && !empty($input['dashboard_padding'])) {

            foreach ($input['dashboard_padding'] as $key => $v) {

                $obj = \App\MyThemeVersion::where('id', $key)->first();

                if ($obj != null) {

                    $obj->dashboard_padding = $v;
                    $obj->save();
                }
            }
        }
    ...
    public function deletemedia(Request $request)
    {
        ...
        $bs = BasicSetting::where('id', $input['id'])->firstOrFail();

        if (!empty($bs->breadcum_video)) {

            @unlink('assets/front/img/breadcrumb/' . $bs->breadcum_video);
            $bs->breadcum_video = Null;
            $bs->save();
        }
