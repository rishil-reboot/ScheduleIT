jQuery(document).ready(function ($) {
    ...
    $('#inboxWebmail_table_lbl').on('click', "#add_lbl", function (e) {
        $('tbody').append('<tr class="add_row"><td><input type="text" class="form-control" name="lbl_name[][]" value="" required="required" /></td><td><input type="text" class="form-control jscolor" name="lbl_code[][]" value="" required="required" /></td><td class="text-center"><button type="button" class="badge badge-danger delc" id="delete_lbl" title="Delete label">X</button></td><tr>');
        e.preventDefault();
    });
...
$(document).on('click', "#doSync", function () {

    var sync = $('#syncType').val();
    ...
    $.ajax({
        ...
        error: function (jqXHR, exception) {
            ...
            $(this).html('<i class="zmdi zmdi-refresh"></i>  Sync ');
        },
    });
    return false;
